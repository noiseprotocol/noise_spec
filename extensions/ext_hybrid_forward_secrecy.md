 ...
 * For `"f"`: If `f` is empty, then sets `temp` to the next `FLEN1` + 16`
   bytes of the message if `HasKey() == True`, or to the next `FLEN1` bytes
   otherwise.  If `f` is not empty, then sets `temp` to the next `FLEN2 + 16`
   bytes of the message if `HasKey() == True`, or to the next `FLEN2` bytes
