 ...
 * **`GENERATE_KEYPAIR_F(r)`**: Generates a new key pair for the
 ...
 * **`FLEN1`** = A constant specifying the size in bytes of the output
   from `GENERATE_KEYPAIR_F(r)` when `r` is empty.
 ...
 * `GENERATE_KEYPAIR_F(r)` = `GENERATE_KEYPAIR()`
 ...
 * `GENERATE_KEYPAIR_F(r)` = `newhope_keygen()` if `r` is empty, or
   `newhope_sharedb(r)` if `r` is not empty.
...
motivated the addition of `r` to the parameters for `GENERATE_KEYPAIR_F()`
